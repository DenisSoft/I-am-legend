...
export class ZombiesSet extends ObjectsSet {
    ...
    static isZombieAttack(zombie, hero) {
    ...
    static isFakeDeadUp(zombie, hero) {
    ...
    static isSlideUnderZombie(zombie, hero) {
    ...
    checkZombiesStatus(hero, game, lieZombiesArr) {
        ...
        this.data.forEach((zombie) => {
            if(!zombie.isDead && !zombie.isFell) {
                if (hero.states.swordAttackState.isCurrent && ZombiesSet.isSwordInZombie(zombie, hero)) {
                    zombie.makeDead(this, game);
                } else if (hero.states.slideState.isCurrent &&  ZombiesSet.isSlideUnderZombie(zombie, hero)) {
                    zombie.currentState = zombie.states.deadState;
                    zombieFellSound.play();
                    zombie.isFell = true;
                    zombie.isDead = true;
                } else if (!hero.isImmortal && !hero.states.slideState.isCurrent && ZombiesSet.isZombieAttack(zombie, hero)) {
                    ...
                    if (ZombiesSet.isCanKillHero(zombie, hero)) {
                ...
                if (ZombiesSet.isFakeDeadUp(zombie, hero)) {
    ...
    makeReincarnation(time, zombiesArr, hero, lieZombiesArr) {
        ...
        function reincarnation() {
            if (this.states.deadState.currentSpriteImg === 0) {
                ...
                this.fakeDead = false;
                this.riseUp = false;
                this.isLeft = this.positionX >= hero.positionX;
                this.currentState = this.states.stayState;
                zombiesArr.add(this);
